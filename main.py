...
class CompressionModel:
    ...
    def configspace(self) -> ConfigurationSpace:
        ...
        batch_size = Integer("batch_size", (8, 32), default=8)
        learning_rate = Float("learning_rate", (0.0001, 0.00015), default=0.0001, log=True)
        lambda_rd = Float("lambda_rd", (0.015, 0.020), default=0.015, log=True)
        test_batch_size = Integer("test_batch_size", (8, 32), default=16)
        cs.add_hyperparameters([batch_size,learning_rate,lambda_rd,test_batch_size])
        return cs
    
    def __init__(self):
        # 设置超参数
        self.model_name = "cheng2020-anchor"
        #self.dataset_path = "/home/gg/Downloads/small_imagenet"
        #self.dataset_path = "/home/gg/py/mnist_png"
        #self.dataset_path = "/content/tiny-imagenet-200"
        self.patience = 2
    ...
    def train(self,config,seed=None):
        self.reset_model(config["learning_rate"], config["lambda_rd"])
        self.batch_size = config["batch_size"]
        self.test_batch_size = config["test_batch_size"]
        log_data = []
        start_time = time.time()
        config_id = f"config_{config['batch_size']}_{config['learning_rate']:.5f}_{config['lambda_rd']:.5f}_{config['test_batch_size']}"
